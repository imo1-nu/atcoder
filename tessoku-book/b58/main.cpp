...
int main()
{
    ...
    tr.update(0, 0);
    for (ll i = 1; i < n; i++) {
        auto itr1 = lower_bound(a.begin(), a.end(), a[i] - l + 1) - 1 - a.begin();
        auto itr2 = lower_bound(a.begin(), a.end(), a[i] - r) - a.begin();
        if (itr1 < 0) itr1 = 1;
        ll min = tr.query(itr2, itr1 + 1);
        tr.update(i, min + 1);
    }
    cout << tr.tree[tr.left + n - 1] << endl;
}